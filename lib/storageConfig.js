...
export function getEventPhotoUrl(storagePath) {
  ...
  if (!baseUrl) {
    safeLog.error('❌ NEXT_PUBLIC_SUPABASE_URL non défini')
    return ''
  }
...
export async function checkBucketExists(supabaseClient) {
  try {
    ...
    if (error) {
      safeLog.error('❌ Bucket Storage manquant:', error.message)
      safeLog.error('ℹ️  Voir docs/ACTIONS-REQUISES-STORAGE.md pour créer le bucket')
      return false
    }
    
    return !!data
  } catch (err) {
    safeLog.error('❌ Erreur vérification bucket:', err.message)
    return false
  }
}
